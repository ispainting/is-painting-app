...
export function JobExpenseEntry({ jobId, jobName, onSaved }: Props) {
  ...
  const extract = api.expenses.extractReceipt.useMutation({
    onSuccess: (result) => {
      setReading(false);
      setExtractionError(result.status === "failed" ? result.message : null);
      if (result.data) {
    ...
    onError: () => { const message = "Receipt reading is temporarily unavailable. The receipt is attached; retry reading or enter the expense manually."; setReading(false); setExtractionError(message); toast.error(message); },
  });
  ...
  return <>
    ...
    {open && <section className="mt-4 w-full min-w-0 max-w-full overflow-hidden rounded-lg border border-brand-200 bg-brand-50/40 p-4" aria-label="Add receipt or expense">
      ...
      {upload && <div className="mt-3 rounded-md border border-slate-200 bg-white p-3"><div className="flex items-center justify-between gap-2 text-sm"><span className="truncate">{upload.file.name}</span><span>{upload.status === "uploading" ? `${upload.progress}%` : upload.status === "success" ? "Attached" : "Failed"}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full bg-brand-600" style={{ width: `${upload.progress}%` }} /></div>{upload.error && <p className="mt-2 text-sm text-red-600">{upload.error}</p>}{reading && <p className="mt-2 text-sm text-brand-700">Reading receipt with AI...</p>}{extractionError && <p className="mt-2 text-sm text-amber-700">{extractionError}</p>}{upload.attachmentId && <div className="mt-3 flex flex-wrap items-center gap-2"><a className="btn btn-secondary min-h-11 text-xs" href={`/api/expenses/attachments/${upload.attachmentId}/preview`} target="_blank" rel="noreferrer">View Receipt</a>{extractionError && <button className="btn btn-secondary min-h-11 text-xs" type="button" disabled={reading || extract.isPending} onClick={() => { setReading(true); setExtractionError(null); extract.mutate({ attachmentId: upload.attachmentId! }); }}>Retry reading</button>}<button className="btn btn-secondary min-h-11 text-xs" type="button" disabled={removeAttachment.isPending} onClick={() => removeAttachment.mutate({ id: upload.attachmentId! })}>Remove / Change</button></div>}</div>}
