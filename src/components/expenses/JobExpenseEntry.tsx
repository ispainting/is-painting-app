...
export function JobExpenseEntry({ jobId, jobName, onSaved }: Props) {
  ...
  return <>
    ...
    {open && <section className="mt-4 rounded-lg border border-brand-200 bg-brand-50/40 p-4" aria-label="Add receipt or expense">
      ...
      <div className="mt-4 grid gap-2 sm:flex"><button type="button" className="btn btn-primary min-h-12 flex-1" onClick={() => cameraInput.current?.click()}>Take Photo</button><button type="button" className="btn btn-secondary min-h-12 flex-1" onClick={() => fileInput.current?.click()}>Choose Photo or PDF</button></div>
      ...
      <div className="mt-4 grid gap-3"><label className="label">Vendor<input className="input mt-1 min-h-11" value={form.vendor} onChange={event => setForm(current => ({ ...current, vendor: event.target.value }))} /></label><label className="label">Category<select className="input mt-1 min-h-11" value={form.category} onChange={event => setForm(current => ({ ...current, category: event.target.value as Category }))}>{CATEGORIES.map(category => <option key={category} value={category}>{category.replaceAll("_", " ")}</option>)}</select></label><label className="label">Amount<input className="input mt-1 min-h-11" type="number" min="0.01" step="0.01" value={form.amount} onChange={event => setForm(current => ({ ...current, amount: event.target.value }))} /></label><label className="label">Date<input className="input mt-1 min-h-11" type="date" value={form.date} onChange={event => setForm(current => ({ ...current, date: event.target.value }))} /></label><label className="label">Description<input className="input mt-1 min-h-11" value={form.description} onChange={event => setForm(current => ({ ...current, description: event.target.value }))} /></label></div>
