...
export function computeDraftProposalEstimateSummary(input: {
  ...
  const summary = computeProposalEstimate({
    workItems: Array.from(workItemsByKey.values())
      ...
      .map((section) => ({
        ...
        laborLines: section.laborLines.map((line) => ({
          ...
          hourlyCost: parseNonNegative(line.hourlyCost, input.defaults.defaultLaborCostRate && input.defaults.defaultLaborCostRate > 0 ? input.defaults.defaultLaborCostRate : 23),
    ...
    settings: {
      ...
      generalLiabilityPercent: parseDraftNumber(input.pricing.generalLiabilityPercent) ?? (input.defaults.defaultGlPercent && input.defaults.defaultGlPercent > 0 ? input.defaults.defaultGlPercent : 1),
