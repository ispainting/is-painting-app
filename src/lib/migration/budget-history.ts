...
import type { JobBudgetChangeHistoryItem } from "../job-financials/types";
...
export function buildBudgetAuditRows(args: {
  userId: number;
  jobId: number;
  ...
  return BUDGET_FIELDS
    ...
    .map((field) => ({
      ...
      entityType: "job",
      entityId: args.jobId,
      before: { field, value: money(args.previousBudgets[field]), changeSetId: args.changeSetId } as Prisma.InputJsonValue,
      after: { field, value: money(args.nextBudgets[field]), changeSetId: args.changeSetId } as Prisma.InputJsonValue,
    }));
}
...
export function parseBudgetHistoryItem(log: {
  ...
  const field = String(after?.field ?? before?.field ?? "");
  if (!BUDGET_FIELDS.includes(field as BudgetField)) return null;
